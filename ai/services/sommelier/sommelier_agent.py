...
async def rewrite_wine_query(wine_name: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a Master Sommelier. Correct the typo or abbreviation in this wine name to its full, standard official name (preferably in English) so it can be accurately searched in a database. Just provide the refined name. Example: '마고 15' -> 'Chateau Margaux 2015'"),
...
async def gather_context_node(state: AgentState):
    """
    [데이터 수집 노드] 
    현재는 작동 테스트를 위해 기존 로직을 주석 처리하고 고정된 데이터를 반환합니다.
    """
    print("--- [NODE: gather_context (TEST MODE - HARDCODED)] ---")

    # --- [테스트용 고정 데이터 시작] ---
    user_pref = {"style": "Heavy Red", "pref": "Bold & Tannic, Love Oak flavors"}
    friend_prefs = [
        {"name": "지훈", "style": "Crisp White", "pref": "Refreshing, High Acidity"},
        {"name": "민수", "style": "Sparkling", "pref": "Sweet and Bubbly"}
    ]
    extracted_menu = {
        "wines": ["Chateau Margaux 2015", "Cloudy Bay Sauvignon Blanc", "Moet & Chandon"],
        "foods": ["Ribeye Steak", "Seafood Pasta", "Truffle Fries"]
    }
    wine_details = [
        {"name": "Chateau Margaux 2015", "type": "Red", "body": 5, "tannin": 4, "sweetness": 1},
        {"name": "Cloudy Bay Sauvignon Blanc", "type": "White", "body": 2, "tannin": 1, "sweetness": 1},
        {"name": "Moet & Chandon", "type": "Sparkling", "body": 3, "tannin": 1, "sweetness": 2}
    ]
