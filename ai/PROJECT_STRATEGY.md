...
### [2026-03-19] 후순위 고도화 과제: Food-Wine Semantic Narrative Pipeline (Planned)
- **배경:** 단순 `food_pairings` 태그(예: "Pork")만으로는 사용자의 구체적인 요리(예: "매콤한 제육볶음")와 와인 간의 미세한 궁합을 판단하기에 정보의 밀도가 부족함.
- **핵심 전략:** 와인의 맛 지표(`taste_profile`), 유저 리뷰(`taste_notes`), 표준 페어링을 융합하여 **'페어링 서사(Pairing Narrative)'**를 생성하고 이를 임베딩의 원천 데이터로 활용.
- **구현 계획:** 
  1.  **Narrative Synthesis:** LLM을 사용하여 각 와인에 대해 "이 와인은 산도가 높아 기름진 삼겹살의 느끼함을 잡아주며, 오크 풍미가 숯불 향과 조화롭습니다"와 같은 **논리적 페어링 문장**을 생성.
  2.  **Semantic Mapping:** 사용자가 입력한 구체적인 음식 명칭을 위 서사와 벡터 공간에서 매칭하여, 단순 재료 일치를 넘어선 **맛의 조화(Harmony)** 기반의 유사도 검색 수행.
  3.  **Explainable AI:** 에이전트가 답변 시 단순히 추천만 하는 것이 아니라, 임베딩된 서사를 인용하여 "왜 이 와인이 이 음식과 어울리는지"에 대한 **전문가 수준의 근거**를 함께 제시.
- **기대 효과:** DB에 없는 창의적인 안주 메뉴에 대해서도 유연하게 대응 가능하며, 사용자의 구체적인 입맛(맵기, 기름진 정도 등)을 와인의 성격과 완벽하게 매칭.
