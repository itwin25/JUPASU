...
- **세션 관리:** `session_id`를 도입하여 사용자별 독립적인 대화 맥락(Context) 유지 및 추적.
- **메모리 최적화 (Context Management):**
  - **문제 인식:** 대화 기록 무제한 누적 시 컨텍스트 윈도우 초과, 비용 증가, 모델 응답 지연 발생 가능성.
  - **해결 방안:**
    1. **슬라이딩 윈도우:** 최신 N개의 메시지만 유지하여 고정된 연산량 확보.
    2. **대화 요약(Summarization):** 특정 토큰 수 초과 시 이전 대화를 요약하여 압축된 맥락 정보만 전달 (Token Efficiency).
- **LangGraph 연동:** LangGraph의 상태(State) 관리 기능을 활용하여 "토큰 체크 -> 조건부 요약 -> 답변 생성"으로 이어지는 **Self-Cleaning Memory** 파이프라인 설계.
