...
- **Query Refinement:** 사용자의 비정형화된 입력(질문 + OCR + 친구 언급)을 LLM이 1차 분석하여 **전문적인 소믈리에 지시문(`RefinedInput`)**으로 정제하는 전방 레이어 구축.
- **구조화된 입력 체계:** 날것의 입력을 `raw_input`, `raw_ocr`로 명확히 구분하여 수집하고, AI가 이를 지능적으로 재구성하여 후속 노드에 전달함으로써 데이터 경계와 맥락을 명확히 함.
- **AI 동적 오케스트레이션:** 정제된 데이터를 기반으로 분석 대상(나/친구들)과 메뉴 분석 필요성 등을 모델이 스스로 판단하여 실행 태스크 리스트를 동적으로 생성.
- **효과:** 사용자 발화의 모호성을 제거하고 에이전트의 의사결정 정확도를 수직 상승시킴으로써, 복잡한 다자간 페어링 시나리오에서도 안정적인 서비스 제공 가능.
