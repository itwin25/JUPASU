...
export default function Scanner() {
  ...
  // 공통 OCR 처리 로직
  const processOCRResults = useCallback(
    async (imageSource: HTMLCanvasElement | HTMLImageElement) => {
      const width =
        imageSource instanceof HTMLCanvasElement ? imageSource.width : imageSource.naturalWidth;
      const height =
        imageSource instanceof HTMLCanvasElement ? imageSource.height : imageSource.naturalHeight;
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            ocrResults.forEach((res) => {
              ...
              if (res.text) {
                const box = res.box as any;
                const x = Array.isArray(box[0]) ? box[0][0] : box.x || box[0];
                const y = Array.isArray(box[0]) ? box[0][1] : box.y || box[1];
