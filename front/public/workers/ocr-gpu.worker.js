// [GPU] 워커 로드 즉시 생존 신고
self.postMessage({ type: 'LOG', payload: 'GPU 워커 스크립트 로드 성공' });

// WebGPU를 지원하는 전체 번들 로드
importScripts('/workers/ort.all.min.js');
...
function getBoxesFromHeatmap(heatmap, hWidth, hHeight, origW, origH, threshold = 0.3) {
  ...
  for (let i = 0; i < heatmap.length; i++) {
    if (heatmap[i] > threshold && visited[i] === 0) {
      ...
      while (queue.length > 0) {
        ...
        if (cy < minY) minY = cy;
        if (cy > maxY) maxY = cy;
        const left = curr - 1, right = curr + 1, up = curr - hWidth, down = curr + hWidth;
  ...
  const scaleX = origW / hWidth, scaleY = origH / hHeight;
  return initialBoxes.map(rect => {
    const vPad = Math.round(rect.h * 0.1), hPad = Math.round(rect.w * 0.05);
    return { 
      x: Math.max(0, (rect.x - hPad) * scaleX), 
      y: Math.max(0, (rect.y - vPad) * scaleY), 
      w: Math.min(origW - (rect.x - hPad) * scaleX, (rect.w + 2 * hPad) * scaleX), 
      h: Math.min(origH - (rect.y - vPad) * scaleY, (rect.h + 2 * vPad) * scaleY) 
    };
...
self.addEventListener('message', async (e) => {
  ...
  try {
    if (type === 'INIT') {
      ...
      // [WebGPU] 실행 프로바이더 우선순위 설정
      const options = { 
        executionProviders: ['webgpu', 'wasm'], 
        graphOptimizationLevel: 'all' 
      };
      
      logToMain('GPU 가속 모델 로딩 시작...');
    ...
    if (type === 'PROCESS') {
      ...
      let ratio = width > height ? (width > MAX_SIDE_LEN ? MAX_SIDE_LEN / width : 1) : (height > MAX_SIDE_LEN ? MAX_SIDE_LEN / height : 1);
      ...
      for (let i = 0; i < boxes.length; i++) {
        const box = boxes[i], targetH = 48, targetW = Math.max(48, Math.floor((box.w / box.h) * targetH));
        const recCanvas = new OffscreenCanvas(targetW, targetH);
        recCanvas.getContext('2d').drawImage(tempCanvas, box.x, box.y, box.w, box.h, 0, 0, targetW, targetH);
        const recResized = recCanvas.getContext('2d').getImageData(0, 0, targetW, targetH).data;
        ...
        const decoded = decode(recResults[recSession.outputNames[0]].data, recResults[recSession.outputNames[0]].dims);
        
        // 개별 박스 인식 결과 로그 출력
        logToMain(`[Box #${i+1}] 인식: "${decoded.text}" (점수: ${(decoded.score).toFixed(3)})`);
        
        if (decoded.text && decoded.score >= 0.7) {
      ...
      self.postMessage({ 
        ...
        payload: { text: finalResultText, score: finalTexts.length > 0 ? scoresSum / finalTexts.length : 0 } 
      });
    }
  } catch (error) {
